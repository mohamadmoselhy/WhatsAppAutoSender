...
def ask_user_to_send_message():
    ...
    unblock_input()  # Ensure this function is defined elsewhere
...
def close_whatsapp_tab():
    """Closes only the WhatsApp Web tab without closing the entire application."""
    try:
        ...
        for win in windows:
            if "WhatsApp Web" in win.title or "WhatsApp" in win.title:
                ...
                win.close()
                return True  # Close only one tab and exit

        print("[INFO] No WhatsApp Web browser tab found.")
        return False
...
def write_in_field(name_field_image, text_value):
    """Finds the contact search field and pastes the Arabic name correctly."""
    try:
        ...
        if name_location:
            ...
            # ✅ Fix Arabic text before pasting
            fixed_text = text_value

            print(f"[INFO] Pasting name: {fixed_text}")
            pyperclip.copy(fixed_text)  # Copy corrected Arabic text to clipboard
